...
- Dead code deleted (~1,100 lines): the unused wizard (pages + CSS), `api.configSchema` helper, plugins loading template, never-populated nav badges, dead `statusBadge` mapping; 401 responses now dispatch a `tram:unauthorized` event (login overlay) instead of dead-ending; health-port fallback uses `window.location.origin`
