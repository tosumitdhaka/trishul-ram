...
| `TRAM_DLQ_SPOOL_DIR` | `~/.tram/dlq-spool` | Local directory where DLQ envelopes are spooled when the DLQ sink write itself fails (v1.4.7, GH #55). Spooled JSON envelopes can be re-fed to the DLQ manually; a failure to even spool is counted in metrics and logged as ERROR — DLQ records are never silently dropped |
